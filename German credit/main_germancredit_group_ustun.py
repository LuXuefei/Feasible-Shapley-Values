# -*- coding: utf-8 -*-
"""
Created on Mon Apr 29 12:43:04 2024
...
# x1 = ep.cf_examples_list[0].final_cfs_df.drop(columns = 'GoodCustomer')   
# x1 = x1.drop(columns = 'GoodCustomer')
# model.predict(x1)
...
for baseind in range(X_target_valid.shape[0]):
    ...
    if ~x1.isnull().values.any(): # if cf not null   
        ...
        # # ~ 855 sec
        # start_time = time.time()
        # _,_ , _, _, phiid, _, phishapeid = JointIndiShapley.finitechangesIndfull(x0,x1,f,threshold, scoreind = 1)
        # time_cal= time.time()- start_time
